...
#' can work interactively by allowing to pick a starting pixel within each pattern element from which the
...
#' @param focal Blur image for color extraction (default = FALSE).
    ...
    if(todo != 'r' && todo != 'a'){
      ...
      if(!is.null(maskOutline)){
        ...
        if(transformRef[1] != 'meanshape'){
          imageTr <- maskOutline(imageTr, maskOutline, refShape = 'target', crop = crop,
